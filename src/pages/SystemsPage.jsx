...
const systems = [
  {
    key: 'synthex', tag: 'green', tagLabel: 'Automation Platform', name: 'SyntheX Prime',
    tagline: 'A 500+ command Discord automation platform. Started as a monolith, ended up as a lesson in what modular architecture actually means once a codebase gets large enough to fight back.',
    props: ['Command registry where modules self-register at startup, dispatcher resolves by name only','Async execution pipelines so slow operations never block the command loop','JSON config separates behaviour from code so the system is tuneable without a deploy'],
  },
  {
